...
The Gradient Editor has been modernized with a new JavaFX interface, providing a more responsive
...
==== _Easy Movie Maker UI (JavaFX Modernization)_
The Easy Movie Maker interface has been completely modernized to JavaFX to provide a smoother, more responsive experience.

===== _Motion Curve Editing_
A core feature of the new interface is the Motion Curve editor. This visual editor allows you to precisely control how animations progress over time.
 - Click the **Edit Curve** button next to a motion to open the `CurveEditorDialog`.
 - You can add, move, and delete control points to shape the animation curve.
 - Double-clicking a point toggles its interpolation mode (e.g., Smooth, Linear).

===== _Movie Rendering_
The rendering logic has been fully integrated into the new controller, replacing the deprecated `TinaSWFAnimatorController`.
 - Use the **Render Movie** button to configure and start the final rendering process.
 - The rendering uses the central JWildfire render engines (CPU or GPU, depending on your settings) to produce high-quality image sequences.
...
==== Motion Curve Editor
For even more control, you can define a *Motion Curve* for each script.
Click the *Curve* button next to the script value to open the Motion Curve Editor.
Here you can define keyframes (Frame, Value) to vary the strength of the effect over time.
For example, you can make the camera rotate slowly at first, then speed up, and then stop.
...
=== Dancing Flames UI (JavaFX Modernization)
The Dancing Flames module has been updated to use JavaFX, bringing performance and workflow improvements.

==== Project Management
You can now save and load your Dancing Flames setups as projects.
 - Click **Load Project** to restore a `.jwfdance` project file, keeping all your mapped motions and loaded flames intact.
 - Click **Save Project** to dump your current setup to a `.jwfdance` project file for later editing.

==== Random Flame Generation
Instead of manually creating flames to dance, you can auto-generate a pool of them:
 1. Pick a base generator style from the **Generator** dropdown.
 2. Specify the number of flames to create in the **Count** field.
 3. Click **Gen Random** to automatically populate your pool with new variations ready for animation.
...
The Batch Renderer has been modernized with a new JavaFX interface, providing a streamlined workflow
for queuing and rendering large sets of flames.
...
The Quilt Renderer features a modernized JavaFX interface that automatically detects existing tiles.
If a render is interrupted, simply start it again with the same settings, and it will automatically
resume where it left off, skipping already rendered tiles.
...
The new modernized interface separates these steps clearly:
1.  **Generate Slices**: Creates the voxel stack images.
2.  **Generate Mesh**: Converts the generated images into a 3D mesh.
...
The GPU Render module has been modernized with a new JavaFX interface, providing a cleaner and more
